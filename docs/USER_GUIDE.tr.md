...
3. [Build Sırasında Ne Üretilir?](#build-sırasında-ne-üretilir)
4. [Ne Zaman Kullanılır?](#ne-zaman-kullanılır)
5. [Gerçek Hayat Senaryoları](#gerçek-hayat-senaryoları)
6. [Hızlı Başlangıç](#hızlı-başlangıç)
7. [Kopyala-Yapıştır API Örnekleri](#kopyala-yapıştır-api-örnekleri)
8. [Endpoint'ler](#endpointler)
9. [Profile Seçimi](#profile-seçimi)
10. [DB Pool Ve c64/c256 Ne Demek?](#db-pool-ve-c64c256-ne-demek)
11. [ZooKeeper Mi Static Service DNS Mi?](#zookeeper-mi-static-service-dns-mi)
12. [Sık Hatalar](#sık-hatalar)
...
## Build Sırasında Ne Üretilir?

Dubbo client wrapper'ları elle yazılmaz. `@GenerateNativeDubboClient`, interface'e göre doğrudan client
metotları üretir. Processor yalnızca Maven build sırasında çalışır. Interface metotları client
oluşturulurken bir kez çözülür. Request sırasında proxy veya reflection ile metot aranmaz.

Maven build ayrıca `META-INF/reactor/components.idx` ve `META-INF/reactor/routes.idx` dosyalarını
üretir. Framework startup sırasında bu listeleri kullanır. Geniş classpath taraması gerekmez. Index
dosyalarını elle düzenlemeyin. Route çakışması varsa build hata verir.
...
```properties
reactor.rust.route-budget.rpc-customer-raw-create.route-admission.max-concurrent=4
reactor.rust.route-budget.rpc-customer-raw-create.route-admission.queue-timeout-ms=250
reactor.rust.route-budget.rpc-customer-typed-create.route-admission.max-concurrent=4
reactor.rust.route-budget.rpc-customer-typed-create.route-admission.queue-timeout-ms=250
```

Named budget aynı iş sınıfındaki route'lara ortak başlangıç değeri verir. Yalnızca bir endpoint farklı
davranıyorsa `reactor.rust.route-admission.<route-key>.*` ile o endpoint'i override edin. Öncelik sırası
annotation < workload < named budget < route-specific property şeklindedir.
...
reactor.rust.route-budget.rpc-customer-raw-read.route-admission.max-concurrent=2
reactor.rust.route-budget.rpc-customer-raw-read.route-admission.queue-timeout-ms=75
```
